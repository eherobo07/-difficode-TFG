...
function validarRespuesta() {
  ...
  const idEjercicio = window.location.pathname.split("/").pop();

  if (!respuesta) {
    mensajeDiv.textContent =
      "Por favor, escribe una respuesta antes de validar.";
    mensajeDiv.className = "resultado incorrecto";
    mensajeDiv.style.display = "block";
    return;
  }

  fetch("/api/ejercicio/validar", {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      if (data.error) {
        ...
        mensajeDiv.className = "resultado correcto";
      }

      mensajeDiv.style.display = "block";
    })
    .catch((err) => {
      mensajeDiv.textContent = "Error al validar. Intenta de nuevo.";
      mensajeDiv.className = "resultado incorrecto";
      mensajeDiv.style.display = "block";
    });
}
